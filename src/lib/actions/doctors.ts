...
export async function getDoctors() {
    try {
        const doctors = await prisma.doctor.findMany({ // find every single doctor in the db
            include: {
                _count: { select: { appointments: true } }, // count the amount of appointments for every single doctor
            },
            orderBy: { createdAt: "desc" }, // show in a descending order
        });
...
export async function updateDoctor(input: UpdateDoctorInput) {
  try {
    ...
    // if email is changing, chek if the new email already exists
